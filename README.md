...
- **独立 `vanillablock` 控制台命令**：替代原 `spawn vanilla_on/off`，不再依附于 spawn 命令
  ...
  - 支持 Tab 自动补全（子命令 + 物品ID + true/false）

### v1.2.3.0

- **新增 Spike Tactical Dynacomp 膛口制退器**（`dynacomp`）：AKM 专属枪口装置，后坐力 -8%
- **新增 Zenit DTK-1 膛口制退器**（`dtk1`）：AKM 专属枪口装置，后坐力 -12%、无枪口火光
- **新增 TDI AKM-L 护木（电镀红）**（`akml`）：AKM 专属护木，需 Leatherman 工具钳安装
- **武器配件地堡 + 武器物资箱**：35x12 实验室风格地堡（双层防弹加强钢墙体），门禁刷卡进入，室内随机武器箱（30% 掉枪、掉 2~4 配件）；新增 Blue Area 地堡（医疗物资，需专用钥匙卡）
- **瞄准速度数据调整**：多个弹匣/激光/瞄具的瞄准速度平衡性调整
- **枪械属性面板瞄准速度显示精确到小数点后 2 位**

### v1.2.2.0

- **新增 Leatherman 多功能工具钳**（`leatherman`）：部分配件安装的前置工具，可合成
- **新增 Magpul MOE AKM 护木 (FDE)**（`moeakm`）：握把/战术手电/激光的安装前提
- **配件工具钳检测机制**：改枪面板无工具时按钮禁用并显示黄色提示
- **配件安装前提机制**：配件之间的依赖关系管理（如握把依赖护木）
- **配件视觉局部叠加**：透明背景配件局部贴图，任意数量配件可同时显示
...
| 12 个自定义弹匣 | 弹匣供弹枪械的专属弹匣，可合成、世界生成 |
...
| 武器配件地堡 | 实验室风格地堡（Weapon Area / Blue Area），门禁刷卡进入，武器箱掉落枪械与配件 |
...
| 15 | **AA-12** | `aa12` | 12g | 全自动 | 弹匣 | 20 | 2.5 | 44 | 30 | 3.3 | 8弹丸 |
...
| 12 | AA-12 弹鼓 | `aa12_mag` | AA-12 | 12g85 | 20 | 25 |
...
部分配件需要 Leatherman 工具钳才能安装（改枪面板会提示），护木是握把/战术手电/激光的安装前提。

| # | 配件 | ItemKey | 类型 | 效果 |
|---|------|---------|------|------|
| 1 | Leatherman 多功能工具钳 | `leatherman` | 工具 | 配件安装前置工具，可合成 |
| 2 | Magpul MOE AKM 护木 (FDE) | `moeakm` | 护木 | 后坐力 -1%、每发耐久损耗 -3%，需工具钳 |
| 3 | TDI AKM-L 护木（电镀红） | `akml` | 护木 | 瞄准速度 -0.35s、每发耐久损耗 -3%，需工具钳 |
| 4 | Spike Tactical Dynacomp | `dynacomp` | 枪口 | 后坐力 -8%、瞄准速度 +0.05s |
| 5 | Zenit DTK-1 | `dtk1` | 枪口 | 后坐力 -12%、瞄准速度 +0.1s、无枪口火光 |
