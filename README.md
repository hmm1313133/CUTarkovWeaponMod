...
> **完整数值表：** 枪械、弹药、弹匣、配件、护甲、头盔、背包等详细数值请参阅同目录 `ITEMS.md`。

## 更新日志

### v2.0.0.0

- **独立 `vanillablock` 控制台命令**：替代原 `spawn vanilla_on/off`，不再依附于 spawn 命令
  - `vanillablock list`：显示全局开关与每个原版物品的当前拦截状态
  - `vanillablock on` / `vanillablock off`：全局开启/关闭原版物品拦截（清空单独覆盖）
  - `vanillablock [物品名] true/false`：单独强制拦截/放行指定物品（优先于全局开关）
  - 支持 Tab 自动补全（子命令 + 物品ID + true/false）；多人游戏需确保两人的设置一致，否则会存在同步异常。
- **多人配件/弹匣/战术灯同步**：
  - 配件安装/卸下通过 KrokMP 原始消息通道双向同步，不再依赖 CUCoreLib MultiplayerBridge 的兼容性
  - 客户端与主机之间可互相看到改装结果
  - 自定义弹匣 `currentMagId` 同步，主机装弹匣后客户端不再显示默认贴图/空弹匣
  - 战术手电/激光档位状态同步，他人能看到持枪者的灯光/激光
  - 主机每 3 秒广播全量枪械状态，重连/后加入玩家也能恢复改装、弹匣、配件电量
- **存档保存扩展**：存档通过 CUCoreLib 保存配件列表、当前弹匣、弹匣内子弹、战术灯/瞄具电量、无电池标记
- **武器配件地堡/物资区扩展**：Weapon Area、Blue Area、Red Area 三类刷卡地堡，内含武器箱/医疗物资/高级装备/弹药箱
- **快捷键添加**：现可以在设置界面设置 拉栓、卸弹匣、卸弹匣内子弹、切换瞄具/战术灯档位 的快捷键
- **配件生态扩展**：Leatherman 工具钳、Magpul MOE AKM 护木、TDI AKM-L 护木、Dynacomp、DTK-1、AKM/M4/SKS/Glock/P90/UMP 多系列配件与视觉叠加
- **大量数值平衡与修复**：配件数值、瞄准速度、弹匣价值、世界生成、多人同步与存档等，详见 `CHANGELOG.md`

### v1.2.1.0

- **新增 Ops-Core FAST 护目罩 / 多重打击防弹面罩**：仅可安装在 FAST MT 系头盔，提供减伤与失明/下颚免疫
- **Rys-T 头部保护机制**：45% 免疫下颚脱位、50% 免疫下颚缺失/毁容、75% 免疫眼部失明
- **卡壳概率公式重做**：多段式线性映射（100%~80% 耐久：0%~0.5% 卡壳率 ~ 20%~0%：30%~60%），潮湿不再影响卡壳率
- **枪械伤害调整**：RPD 生物伤 87→100、AA-12 单弹丸 44→41、P90 45→50、格洛克17 50→32
- **EXFIL 头盔加入夜视仪兼容列表**
- **摘盔掉落面罩**：摘下 FAST MT 系头盔时面罩自动掉落
- **修复食物箱刷煮方便面 / 失明免疫体感低于数值**

### v1.2.0.7

- **新增 AA-12 自动霰弹枪**：MPS Auto Assault-12 Gen 1，12g 口径，全自动，20 发弹鼓，8 弹丸散布
- **枪械价值调整**：批量更新 15 把枪械和 2 把近战武器的价值
- **背包/弹挂衰减改为移动时衰减**：21 件背包/弹挂的衰减条件改为仅穿戴且移动时衰减（NoDecayWhenNotWorn | NoDecayWhenStill）
- **修复食物箱生成煮熟方便面**：食物箱不再错误生成隐藏的合成中间物品
- **修复 AA-12 装填/卸下弹匣问题**：基础预制体从 shotgun 改为 rifle，原生继承 feedType=Mag
- **AA-12 枪口/枪管位置微调**
...
- **食物系统**：新增 11 种食物（军用饼干、黑麦面包块、士力架能量棒、塔克肉干、Alyonka 巧克力棒、一包糖、Iskra/MRE 单兵口粮、豌豆罐头、方便面、煮熟方便面）+ 糖水液体 + TK Fast MT 头盔仿制品
- **弹挂甲双向锁定**：修复先穿弹挂再穿弹挂甲可同时穿戴的问题
- **头盔减伤调整**：所有头盔减伤 +25 个百分点
- **cangetwet 清理**：移除所有非枪械物品的 cangetwet 标签
- **食物使用原版战利池生成**
- **修复 item.cont/item.battery 绑定缺失**：35 件物品耐久条/电池卸下异常
- **修复 NVG/耳机卸下后拖拽冻结**：添加 WornSprite
- **修复夜视仪换层后增亮/overlay 消失**
- **修复多人模式客户端无法穿戴夜视仪/耳机**
- **修复食物多次使用耐久异常**
...
- **战术耳塞系统**：新增 2 件入耳式电子防护耳机（TEP-300 / ProFlex DX5），使用全新 `ear` 槽位，与头盔/夜视仪可同时穿戴。有电时提供听力损伤降低、听距增强、主动降噪；无电时听距缩小、环境音量降低。减轻声波炮视觉/听觉影响。
...
- **手枪图标统一**：Glock17/USP 大小和持枪位置与 Deagle 一致
- **合成配方分类**：护甲修复->材料栏、弹匣合成->实用物品栏
- **修复 item.cont 绑定**：30 件容器物品耐久条不随重量变化、衰减时间显示异常
- **修复 item.battery 绑定**：5 件电池物品无法卸下电池
- **修复世界生成链式反应**：自定义物品的 Container.Awake 触发更多物品生成
- **修复 VSS 被原版战利池生成**
- **修复所有自定义物品被原版战利池双重生成**
- **修复原版背包出现在合成列表**
...
| 功能 | 说明 |
|------|------|
| 15 把自定义枪械 | 栓动狙击/半自动卡宾/全自动步枪/泵动霰弹枪/自动霰弹枪/手枪/冲锋枪/轻机枪/消音狙击 |
| 2 把近战武器 | Red Rebel 冰镐（攀爬+切割+捶打）、M-2 战术剑（切割+捶打），附带挥砍音效 |
| 3 件夜视仪 | GPNVG-18/PVS-14/PVS-31A，需兼容头盔，N键开关，开关音效 |
| 2 件战术耳机 | TEP-300 / ProFlex DX5，ear 槽位，听力保护、听距增强与主动降噪 |
| 2 件 FAST 面罩 | Ops-Core 护目罩/多重打击防弹面罩，眼部防护与失明/下颚免疫 |
| 枪械配件系统 | 护木/制退器/激光/手电等配件，工具钳安装校验与配件前提机制，局部叠加视觉 |
...
| 11 种食物 | 军用饼干~煮熟方便面，使用原版战利池生成 |
| 2 种防弹插板 | 普通/高级，用于护甲修复 |
| 10 种自定义弹药 | 每把枪对应专属口径弹药，通过口径系统强制隔离 |
| 17 个自定义弹匣/弹鼓 | 弹匣供弹枪械的专属弹匣/弹鼓，可合成、世界生成 |
...
| 武器配件地堡 | 实验室风格地堡（Weapon Area / Blue Area / Red Area），门禁刷卡进入，武器箱/医疗/高级装备/弹药箱 |
| 枪械音效 | 每把枪有独立的开火/拉栓/闭栓/装弹匣/卸弹匣音效 |
| 合成系统 | 弹药 + 弹匣可通过合成配方制作；7 件护甲可合成，23 件护甲可用插板修复 |
| 原版武器替换 | 封禁原版武器/弹药/弹匣，替换为自定义武器系统；`vanillablock` 命令支持全局开关与单物品覆盖 |
| AXMC 瞄准镜 | AXMC 狙击步枪装备时扩展视野范围 |
| UMP45 消音器 | UMP45 内置消音器，噪音仅 0.2，禁用枪口火光 |
| 控制台 Spawn | 控制台 `spawn` 命令支持所有自定义物品 ID |
| 原版物品开关 | `vanillablock` 独立命令切换原版武器生成/合成/交易（支持单物品覆盖） |
...
| `vanillablock list` | 显示全局开关与每个原版物品的当前拦截状态 |
| `vanillablock on` / `vanillablock off` | 全局启用/禁用原版武器/弹药/弹匣拦截（清空单独覆盖） |
| `vanillablock [物品名] true/false` | 单独强制拦截/放行指定物品（优先于全局开关） |
...
|---|------|---------|------|---------|------|------|------|--------|--------|------|---------|
| 1 | **AXMC** | `axmc` | .338 LM | 栓动 | 弹匣 | 10 | 2.52 | 310 | 299 | 6.0 | 瞄准镜视野扩展 |
| 2 | **DVL-10** | `dvl10` | 7.62x51 | 栓动 | 弹匣 | 10 | 2.22 | 205 | 180 | 4.5 | - |
| 3 | **SKS** | `sks` | 7.62x39 | 半自动 | 直供 | 10 | 2.04 | 150 | 100 | 2.9 | - |
| 4 | **AKM** | `akm` | 7.62x39 | 全自动 | 弹匣 | 30 | 2.1 | 120 | 90 | 3.0 | - |
| 5 | **Desert Eagle** | `deagle` | .50 AE | 半自动 | 弹匣 | 7 | 1.02 | 110 | 60 | 5.5 | - |
| 6 | **M4A1** | `m4a1` | 5.56x45 | 全自动 | 弹匣 | 30 | 2.04 | 90 | 70 | 2.7 | - |
| 7 | **RPD** | `rpd` | 7.62x39 | 全自动 | 弹匣 | 100 | 3.6 | 100 | 70 | 3.0 | 弹链供弹 |
| 8 | **Glock 17** | `glock17` | 9x19 | 半自动 | 弹匣 | 17 | 0.78 | 32 | 20 | 2.0 | - |
| 9 | **P90** | `p90` | 5.7x28 | 全自动 | 弹匣 | 50 | 1.8 | 50 | 35 | 1.9 | - |
| 10 | **UMP 45** | `ump45` | .45 ACP | 全自动 | 弹匣 | 25 | 1.26 | 44 | 27 | 2.2 | 可装 UMP OEM 消音器 |
| 11 | **MP-153** | `mp153` | 12g | 半自动 | 直供 | 8 | 2.34 | 41 | 30 | 4.0 | 8弹丸 |
| 12 | **MP-133** | `mp133` | 12g | 泵动 | 直供 | 4 | 2.28 | 40 | 30 | 4.0 | 8弹丸 |
| 13 | **USP** | `usp` | .45 ACP | 半自动 | 弹匣 | 12 | 0.72 | 40 | 42 | 2.2 | - |
| 14 | **VSS** | `vss` | 9x39 | 全自动 | 弹匣 | 30 | 1.8 | 105 | 88 | 0.32 | 整体式消音器 |
| 15 | **AA-12** | `aa12` | 12g | 全自动 | 弹匣 | 20 | 2.5 | 41 | 30 | 3.3 | 8弹丸 |
...
| 4 | 12/70 8.5mm | `12g85` | 12g | MP-133 / MP-153 / AA-12 |
...
| # | 弹匣/弹鼓 | ItemKey | 对应枪械 | 对应弹药 | 容量 | 价值 |
|---|------|---------|---------|---------|------|------|
| 1 | AXMC 弹匣 | `axmc_mag` | AXMC | 338ucw | 10 | 20 |
| 2 | DVL-10 弹匣 | `dvl10_mag` | DVL-10 | 76251bpz | 10 | 17 |
| 3 | AKM 弹匣 | `akm_mag` | AKM | 76239sp | 30 | 32 |
| 4 | X-47 50发弹鼓 | `x47mag` | AKM | 76239sp | 50 | 55 |
| 5 | 沙漠之鹰弹匣 | `deagle_mag` | Desert Eagle | 50copper | 7 | 14 |
| 6 | Glock 17 弹匣 | `glock17_mag` | Glock 17 | 919pso | 17 | 17 |
| 7 | Glock Big Stick 弹匣 | `bigstick_mag` | Glock 17 | 919pso | 33 | 33 |
| 8 | Glock 50发弹鼓 | `g50_mag` | Glock 17 | 919pso | 50 | 50 |
| 9 | M4A1 弹匣 | `m4a1_mag` | M4A1 | 55645fmj | 30 | 32 |
| 10 | M4A1 60发弹匣 | `mag560` | M4A1 | 55645fmj | 60 | 68 |
| 11 | P90 弹匣 | `p90_mag` | P90 | 5728sb193 | 50 | 55 |
| 12 | UMP 45 弹匣 | `ump45_mag` | UMP 45 | 45fmj | 25 | 26 |
| 13 | USP 弹匣 | `usp_mag` | USP | 45fmj | 12 | 12 |
| 14 | RPD 弹链盒 | `rpd_mag` | RPD | 76239sp | 100 | 110 |
| 15 | VSS 弹匣 | `vss_mag` | VSS | 939sp5 | 30 | 33 |
| 16 | AA-12 弹鼓 | `aa12_mag` | AA-12 | 12g85 | 20 | 25 |
| 17 | SKS-A5 弹匣 | `sks_a5_mag` | SKS | 76239sp | 20 | 35 |
...
|---|------|---------|------|------|---------|------|
| 1 | 武器维修套件 | `weaponrepairkit` | 4.5u | 52 | 4 次 | 重量随耐久减少 |
...
入耳式电子防护耳机，与头盔/夜视仪可同时穿戴。有电时提供听力损伤降低、听距增强、主动降噪；无电时听距缩小、环境音量降低。可减轻声波炮视觉/听觉影响。

| # | 耳机 | ItemKey | 特殊 |
|---|------|---------|------|
| 1 | TEP-300 | `tep300` | 电子防护耳机 |
| 2 | ProFlex DX5 | `proflextac` | 电子防护耳机 |

### FAST 面罩（hat 槽位，仅 FAST MT / TK Fast MT 头盔）

| # | 面罩 | ItemKey | 重量 | 价值 | 智力 | 减伤 | 免疫 |
|---|------|---------|------|------|------|------|------|
| 1 | Ops-Core FAST 护目罩 | `fastvisor` | 0.2u | 20 | 8 | 5% | 30% 眼部失明 |
| 2 | Ops-Core FAST 多重打击防弹面罩 | `fastvisor2` | 0.35u | 32 | 8 | 6% | 40% 眼部失明 + 25% 下颚缺失 |

摘下 FAST MT 系头盔时，眼睛槽位的护目罩/面罩自动掉落到地上。

### 食物（utility 物品，原版战利池生成）

| # | 食物 | ItemKey | 腐坏 | 效果 |
|---|------|---------|------|------|
| 1 | 军用饼干 | `crackers` | 不腐坏 | +3 饱食 / -1 水分 / +0.5 心情 |
| 2 | 黑麦面包块 | `croutons` | 不腐坏 | +6 饱食 / -3 水分 / +1 心情 |
| 3 | 士力架能量棒 | `slickers` | 4小时 | +7 饱食 / -3 水分 / +2 心情 / +22 患病 |
| 4 | 塔克肉干 | `tarker` | 不腐坏 | +6 饱食 / -2 水分 / +1.3 心情 |
| 5 | Alyonka 巧克力棒 | `alyonka` | 2.5小时 | +5 饱食 / -3 水分 / +1.8 心情 / +19 患病 |
| 6 | 一包糖 / Iskra / MRE 单兵口粮 / 豌豆罐头 / 方便面 / 煮熟方便面 | - | - | 详见游戏内物品介绍 |

### 枪械改装配件总览

> 下表列出当前所有枪械改装槽位与对应配件 ItemKey。详细数值（后坐力倍率、瞄准速度、耐久损耗、电量等）请参阅同目录 `ITEMS.md` 与游戏内改枪面板悬停预览。

#### 工具 / 通用

| 配件 | ItemKey | 说明 |
|------|---------|------|
| Leatherman 多功能工具钳 | `leatherman` | 部分配件安装前置工具，可合成 |

#### 护木 / 套件（互斥）

| ItemKey | 适用 | 说明 |
|---------|------|------|
| `moeakm` | AKM | Magpul MOE AKM，后坐力 -1%，需工具钳 |
| `hexagonak_hg` | AKM | Hexagon AK 管状护木 |
| `b10mb19` | AKM | B-10M + B-19 导轨护木 |
| `wasr` | AKM | WASR 护木（不可装战术设备/前握把） |
| `akml` | AKM | TDI AKM-L，瞄准速度 -0.35s，需工具钳 |
| `moesl` / `viper` / `kacris` | M4A1 | MOE SL / Viper / KAC RIS 普通护木 |
| `smrmk16` / `adarwood` / `lvoa` | M4A1（加长枪管） | 长枪管专属护木 |
| `uas_sks` | SKS | UAS 套件 |
| `tapco_intrafuse` | SKS | Tapco INTRAFUSE 套件 |

#### 后托 / 枪托（互斥）

| ItemKey | 适用 | 说明 |
|---------|------|------|
| `opforaa47` / `kocherga` / `zhukovs` | AKM | AKM 后托 |
| `cqr47` | AKM | CQR47 一体式枪托（含后握把） |
| `sks_mc` | SKS | ATI Monte Carlo 枪托 |
| `vipermod1` / `ctr` / `ds150fde` / `acs` / `moefg` / `moefde` / `moesg` | M4A1 | M4 系列后托 |
...
|---------|------|
| `rk3` / `mg47` / `ags74` | AKM |
| `td120001` / `starkarrg` / `miad` / `f1st2pc` / `ergo` | M4A1 |
| `axmc_grip` | AXMC |
...
| `hexagonakm` | AKM | 消音器 |
| `rotor43762` | AKM | 消音器 |
| `dynacomp` | AKM | 制退器，后坐力 -8% |
| `dtk1` | AKM | 制退器，后坐力 -12%、无火光 |
| `srvvakm` / `dtk4m` / `dtkp` | AKM | SRVV / DTK-4M / DTKP |
| `rotor43` / `nt4` / `sakerasr556` | M4A1 | M4 消音器 |
| `kx3` | M4A1 | 消焰器 |
| `vp09` | M4A1 | 制退器 |
| `hexagon_sks` | SKS | 声音抑制器 |
| `wt0032_1` | SKS | 螺纹转换器 |
| `ac858` | AXMC | 制退器 |
| `hekate_dt338` / `tmb338lm` / `tsm338lm` | AXMC | 消音/制退/抑制器 |
| `dvl10_silenced` | DVL-10 | 消音枪管组合 |
| `g3port` / `lw9` / `osprey9` / `srd9` | Glock | 枪口补偿/抑制器 |
| `p90attenuator` | P90 | P90 Attenuator 消音器 |
| `ump_oem` | UMP45 | B&T OEM 消音器 |

#### 枪管 / 套筒 / 基座 / 防尘盖（互斥）

| ItemKey | 适用 | 槽位 |
|---------|------|------|
| `m4longbarrel` | M4A1 | 加长枪管 |
| `awlw` | Glock | AW 螺纹枪管（解锁枪口配件） |
| `vipercut` / `ps9` | Glock | 改装套筒 |
| `um3` | Glock | 瞄具基座 |
| `pdc` | AKM | 导轨防尘盖 |
| `mtu017` | SKS | UTG PRO MTU017 机匣基座 |
...
|---------|------|------|
| `mrs` | 反射式 | MRS |
| `eotech553` | 全息 | EOTech 553 |
| `hhs1` | 倍镜 | EOTech HHS-1 |
| `specterdr` | 倍镜 | ELCAN SpecterDR |
| `monstr2x32` | 倍镜 | Monstrum 2x32 |
| `ta01nsn` | 倍镜 | TA01NSN 4x |
| `razorhd` | 倍镜 | Razor HD |
| `pm2` | 倍镜 | PM II |
| `dp` | 红点 | Leupold DeltaPoint |
| `acrop1` | 红点 | Aimpoint ACRO P-1 |

#### 战术设备（每把枪同时只能装一个）

| ItemKey | 说明 |
|---------|------|
| `lastac2` | LAS/TAC 2 战术手电 |
| `klesch2u` | Klesch-2U 战术手电 |
| `baldrpro` | Baldr Pro 手电/激光组合 |
| `tbl` | TBL 蓝色激光 |

#### 前握把（互斥）

`shift`、`se5`、`rk0`、`rk2`、`b25ur1`、`cobra`、`p2`、`afg`

#### SKS 供弹改件

| ItemKey | 说明 |
|---------|------|
| `sks_integral_mag` | SKS 10 发弹仓（默认 Direct 模式） |
| `sks_a5_mag` | SKS-A5 20 发弹匣（Mag 模式） |

### 武器配件地堡（世界生成建筑）

| 地堡 | 进入方式 | 内部物资 |
|------|---------|---------|
| Weapon Area | 门禁刷卡（`weaponroom_keycard`，每次损耗 2%） | 每房间 2~3 个小武器箱 + 1 个大型武器箱（30% 掉枪、掉 2~4 配件） |
| Blue Area | Blue Area 钥匙卡 | SMU06 医疗包、食物箱、自动医疗系统 |
| Red Area | Red Area 钥匙卡 | 大型武器箱、装备物资箱、子弹箱 |

35x12 实验室风格地堡，双层防弹加强钢墙体（挖掘/近战仅 10% 伤害），每深度层生成 3 个（第五地层不刷新）。

## 瞄准 / 配件属性机制

- **瞄准速度**由枪械基础值 + 已装配件 / 当前弹匣共同决定：
  - 弹匣通过 `GunAttachmentHolder.currentMagId` 参与瞄准速度修正（AKM 30发 +0.2s、X-47 +0.5s 等）
  - 配件可提供瞄准速度修正（TDI AKM-L -0.35s、Razor HD +0.5s、PM II +0.7s 等）
  - 改枪面板/悬停显示精确到 0.01s
- **后坐力 / 听力 / 散布 / 耐久损耗**：配件通过乘算倍率叠加，改枪面板悬停可直接预览
- **腰射散布**：握把、枪托、SKS 套件、激光开启等会提供腰射精度乘算；最终腰射精度不会优于枪械基础垂直散布
- **卡壳机制**：按耐久多段线性映射，潮湿不再影响卡壳率
- **护木依赖**：AKM 的前握把 / 战术设备需要护木；SKS 的 UAS / Tapco 会改变可用瞄具、后托等改装
- **SKS 供弹模式**：
  - 默认装有 10 发弹仓改件，Direct 直供
  - 卸下弹仓后可安装 SKS-A5 弹匣，切换为 Mag 模式
- **瞄准镜放大与视野**：倍镜（HHS-1 / SpecterDR / Monstr / TA01NSN / Razor HD / PM II）放大时，AXMC 等枪械会启用扩展视野
...
| SKS + 霰弹枪 | 20% | SKS、MP133、MP153、AA-12 |
| 冲锋枪 | 17% | P90、UMP45 |
| 步枪 | 13% | AKM、M4A1 |
| 狙击枪 | 10% | AXMC、DVL-10 |
| 轻机枪 | 5% | RPD |
...
└── Framework/
    ...
    ├── VanillaCommand.cs                  # 独立 vanillablock 控制台命令（list/on/off/单物品切换）
