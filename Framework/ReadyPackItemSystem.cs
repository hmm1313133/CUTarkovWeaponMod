...
/// <summary>
/// Vertx Ready Pack背包（红色）【ReadyPack】
///
/// 战术日常携行背包，占用 back 槽位，不提供防护。
/// 穿戴时持续消耗耐久，约4.7小时后损坏（destroyAtZeroCondition = true）。
/// 拥有5点可撕裂属性（wearableHitDurabilityLossMultiplier = 5），受击时快速损耗。
/// 通过 CUCoreLib CustomItemInfo 注册，设置 wearable + Container + WornSprite。
/// </summary>
public static class ReadyPackItemSystem
{
    public const string ItemKey = "readypack";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "back";

    public static string DisplayName => I18n.Tr("readypack.name");
    public static string Description => I18n.Tr("readypack.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsReadyPackRequest(MedicalGrantRequest request)
        => request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    // === 数值 ===
    public static float Weight = 0.6f;                        // 重量 0.6u
    public static float WearableIsolation = 0.02f;            // 保温值
    public static int Value = 20;                              // 价值
    public static int RecognitionMin = 2;                      // 识别所需智力
    public static float ContainerCapacity = 4.8f;              // 容器容量 4.8u
    public static float ContainerMaxWeightPerItem = 2.2f;      // 单物品最大重量 2.2u
    public static float ContainerEncumbranceReduction = 0.50f; // 重量减免 50%
    ...
    // === 时间衰减 ===
    // 约4.7小时损坏：4.7 * 3600 = 16920 秒，每秒衰减 1/16920
    public static float DecayRatePerSecond = 1.0f / 16920.0f;

    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        if (!IsReadyPackRequest(request)) return;
        ...
        {
            sr.sprite = icon;
            Plugin.Log.LogInfo($"[ReadyPack] Set sprite to readypack icon ({icon.texture.width}x{icon.texture.height}).");
        }
        else
        {
            Plugin.Log.LogWarning($"[ReadyPack] Icon load failed - will keep base prefab sprite.");
        }

        ResizeColliderToSprite(item);

        Plugin.Log.LogInfo($"[ReadyPack] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        {
            Plugin.Log.LogError($"[ReadyPack] Failed to register '{ItemKey}': {ex}");
            return false;
        }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[ReadyPack] RegisterWithCUCoreLib: icon={icon != null}, wornIcon={wornIcon != null}");
        ...
        Plugin.Log.LogInfo($"[ReadyPack] Configured CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}, Container={customInfo.Container != null}.");
    }
    ...
    public static void TickDecay()
    {
        ...
        if (newCondition <= 0f)
        {
            Plugin.Log.LogInfo($"[ReadyPack] Backpack deteriorated to zero condition, destroying.");
            item.condition = 0f;
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "readypack.png");
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "readypack-icon";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[ReadyPack] Failed to load icon: {ex.Message}");
        }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "readypack-worn";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[ReadyPack] Failed to load worn icon: {ex.Message}");
        }
    ...
    public static class ReadyPackHoverPatch
    {
        [HarmonyPostfix]
