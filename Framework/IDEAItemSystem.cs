...
/// IDEA DIY 胸挂
///
/// 用商场购物袋自制的胸挂，占用 bandolier 槽位，不提供防护。
/// 穿戴时持续消耗耐久，约4小时后损坏（destroyAtZeroCondition = true）。
/// 通过 CUCoreLib CustomItemInfo 注册，设置 wearable + Container + WornSprite。
/// </summary>
public static class IDEAItemSystem
{
    public const string ItemKey = "idea";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "bandolier";

    public static string DisplayName => I18n.Tr("idea.name");
    public static string Description => I18n.Tr("idea.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsIDEARequest(MedicalGrantRequest request)
        => request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    // === 数值 ===
    public static float Weight = 0.2f;                    // 重量 0.2u
    public static float WearableIsolation = 0.02f;        // 保温值
    public static int Value = 16;                          // 价值
    ...
    {
        if (!IsIDEARequest(request)) return;
        ...
        {
            sr.sprite = icon;
            Plugin.Log.LogInfo($"[IDEA] Set sprite to idea icon ({icon.texture.width}x{icon.texture.height}).");
        }
        else
        {
            Plugin.Log.LogWarning($"[IDEA] Icon load failed - will keep base prefab sprite.");
        }

        ResizeColliderToSprite(item);

        Plugin.Log.LogInfo($"[IDEA] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[IDEA] Registered '{ItemKey}' as wearable bandolier (no armor, decays over time).");
            return true;
        }
        catch (Exception ex)
        {
            Plugin.Log.LogError($"[IDEA] Failed to register '{ItemKey}': {ex}");
            return false;
        }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[IDEA] RegisterWithCUCoreLib: icon={icon != null}, wornIcon={wornIcon != null}");
        ...
        Plugin.Log.LogInfo($"[IDEA] Configured CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}, Container={customInfo.Container != null}.");
    }

    // === 时间衰减 ===

    /// <summary>
    /// 每帧调用（由 Plugin.Update 驱动）。
    /// 检查本地玩家是否穿戴 IDEA 胸挂，若是则按时间衰减耐久。
    /// 耐久归零时销毁物品（destroyAtZeroCondition = true）。
    /// </summary>
    public static void TickDecay()
    {
        ...
        if (newCondition <= 0f)
        {
            Plugin.Log.LogInfo($"[IDEA] Shopping bag deteriorated to zero condition, destroying.");
            item.condition = 0f;
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "idea.png");
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "idea-icon";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[IDEA] Failed to load icon: {ex.Message}");
        }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "idea-worn";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[IDEA] Failed to load worn icon: {ex.Message}");
        }
