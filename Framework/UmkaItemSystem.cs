...
/// Umka М33-SET1 猎人背心
///
/// 狩猎背心，占用 bandolier 槽位，不提供防护。
/// 穿戴时持续消耗耐久，约6小时后损坏（destroyAtZeroCondition = true）。
/// 通过 CUCoreLib CustomItemInfo 注册，设置 wearable + Container + WornSprite。
/// </summary>
public static class UmkaItemSystem
{
    public const string ItemKey = "umka";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "bandolier";

    public static string DisplayName => I18n.Tr("umka.name");
    public static string Description => I18n.Tr("umka.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsUmkaRequest(MedicalGrantRequest request)
        => request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    // === 数值 ===
    public static float Weight = 1.7f;                    // 重量 1.7u
    public static float WearableIsolation = 0.14f;        // 保温值
    public static int Value = 31;                          // 价值
    ...
    {
        if (!IsUmkaRequest(request)) return;
        ...
        {
            sr.sprite = icon;
            Plugin.Log.LogInfo($"[Umka] Set sprite to umka icon ({icon.texture.width}x{icon.texture.height}).");
        }
        else
        {
            Plugin.Log.LogWarning($"[Umka] Icon load failed - will keep base prefab sprite.");
        }

        ResizeColliderToSprite(item);

        Plugin.Log.LogInfo($"[Umka] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[Umka] Registered '{ItemKey}' as wearable bandolier (no armor, decays over time).");
            return true;
        }
        catch (Exception ex)
        {
            Plugin.Log.LogError($"[Umka] Failed to register '{ItemKey}': {ex}");
            return false;
        }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[Umka] RegisterWithCUCoreLib: icon={icon != null}, wornIcon={wornIcon != null}");
        ...
        Plugin.Log.LogInfo($"[Umka] Configured CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}, Container={customInfo.Container != null}.");
    }
    ...
    public static void TickDecay()
    {
        ...
        if (newCondition <= 0f)
        {
            Plugin.Log.LogInfo($"[Umka] Vest deteriorated to zero condition, destroying.");
            item.condition = 0f;
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "umka.png");
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "umka-icon";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[Umka] Failed to load icon: {ex.Message}");
        }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "umka-worn";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[Umka] Failed to load worn icon: {ex.Message}");
        }
    ...
    public static class UmkaHoverPatch
    {
        [HarmonyPostfix]
