...
/// <summary>
/// SKS Weapon Tuning 7.62x39 螺纹转换器【WT0032-1】。
/// Weapon Tuning 莫辛步枪螺纹转接器为 SKS 安装现代 7.62x39 膛口装置提供了可能。
///
/// 效果：本身无属性效果，安装后解锁 SKS 的枪口槽——
/// 可安装膛口装置：DTK-1、DTK-4M（后续）、SRVV AKM（后续）、DTKP（后续）。
///
/// 交互：改枪面板（G 键）安装/卸下。
/// </summary>
public static class Wt0032_1ItemSystem
{
    public const string ItemKey = "wt0032_1";
    public const string BaseGameItemId = "bruisekit";

    public static string DisplayName => I18n.Tr("wt0032_1.name");
    public static string Description => I18n.Tr("wt0032_1.desc");
    ...
    public static bool IsWt0032_1Request(MedicalGrantRequest request)
        => request != null && request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        if (!IsWt0032_1Request(request)) return;
        item.id = ItemKey;
        ...
        Plugin.Log.LogInfo($"[WT0032-1] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[WT0032-1] Registered '{ItemKey}' as attachment.");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[WT0032-1] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[WT0032-1] CUCoreLib: Icon={customInfo.Icon != null}.");
    }

    private static Sprite? TryLoadIcon()
    {
        if (_cachedIcon != null) return _cachedIcon;
        _cachedIcon = LoadSprite("WT0032-1.png", "wt0032-1-icon");
        return _cachedIcon;
    }
    ...
    private static Sprite? TryLoadVisualIcon()
    {
        ...
        _cachedVisualIcon = LoadSprite("WT0032-1.png", "wt0032-1-visual");
    ...
    private static Sprite? LoadSprite(string file, string name)
    {
        try
        {
            ...
            else Plugin.Log.LogWarning($"[WT0032-1] Icon not found: {iconPath}");
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[WT0032-1] Icon: {ex.Message}"); }
        return null;
    }
