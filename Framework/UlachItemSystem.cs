...
public static class UlachItemSystem
{
    public const string ItemKey = "ulach";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "hat";

    public static string DisplayName => I18n.Tr("ulach.name");
    public static string Description => I18n.Tr("ulach.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsUlachRequest(MedicalGrantRequest request)
    ...
    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        ...
        if (icon != null && sr != null) { sr.sprite = icon; Plugin.Log.LogInfo($"[ULACH] Set sprite ({icon.texture.width}x{icon.texture.height})."); }
        else { Plugin.Log.LogWarning("[ULACH] Icon load failed."); }
        ResizeColliderToSprite(item);
        Plugin.Log.LogInfo($"[ULACH] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[ULACH] Registered '{ItemKey}' as wearable helmet.");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[ULACH] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[ULACH] CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}.");
    }
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "ulach.png");
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "ulach-icon";
            }
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[ULACH] Icon: {ex.Message}"); }
        return _cachedIcon;
    }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "ulach-worn";
            }
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[ULACH] Worn: {ex.Message}"); }
    ...
    public static class UlachHoverPatch
    {
        [HarmonyPostfix]
