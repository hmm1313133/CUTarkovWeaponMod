...
/// 提供胸部和腹部防护，减伤68%。
/// </summary>
public static class SixB45ItemSystem
{
    public const string ItemKey = "6b45";
    public const string BaseGameItemId = "bruisekit";

    public static string DisplayName => I18n.Tr("6b45.name");
    public static string Description => I18n.Tr("6b45.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool Is6B45Request(MedicalGrantRequest request)
        => request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    // === 数值 ===
    // 减伤55.3%: 1/(1+a) = 0.447, a = 1/0.447 - 1 = 1.2371
    public static float WearableArmor = 1.2371f;
    public static float Weight = 4.4f;                    // 重量 4.4u
    public static float WearableHitDurabilityLossMultiplier = 0.19f; // 被击中耐久损失倍率
    public static float WearableIsolation = 0.11f;        // 保温值
    public static int Value = 64;                          // 价值
    ...
    {
        if (!Is6B45Request(request)) return;
        ...
        {
            sr.sprite = icon;
            Plugin.Log.LogInfo($"[6B45] Set sprite to 6b45 icon ({icon.texture.width}x{icon.texture.height}).");
        }
        else
        {
            Plugin.Log.LogWarning($"[6B45] Icon load failed - will keep base prefab sprite.");
        }

        ResizeColliderToSprite(item);

        Plugin.Log.LogInfo($"[6B45] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[6B45] Registered '{ItemKey}' as wearable vest.");
            return true;
        }
        catch (Exception ex)
        {
            Plugin.Log.LogError($"[6B45] Failed to register '{ItemKey}': {ex}");
            return false;
        }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[6B45] RegisterWithCUCoreLib: icon={icon != null}, wornIcon={wornIcon != null}");
        ...
        downPlaceholder.name = "6b45-down-empty";
        customInfo.MultiWornSprites["DownTorso"] = downPlaceholder;
        Plugin.Log.LogInfo($"[6B45] Set MultiWornSprites: DownTorso (hidden, transparent placeholder).");
        ...
        Plugin.Log.LogInfo($"[6B45] Configured CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}, MultiWornSprites={customInfo.MultiWornSprites?.Count ?? 0}, Container={customInfo.Container != null}.");
    }
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "6b45.png");
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "6b45-icon";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[6B45] Failed to load icon: {ex.Message}");
        }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "6b45-worn";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[6B45] Failed to load worn icon: {ex.Message}");
        }
    ...
    public static class SixB45DualSlotPatch
    {
