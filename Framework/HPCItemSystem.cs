...
public static class HPCItemSystem
{
    public const string ItemKey = "hpc";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "outertorso";

    public static string DisplayName => I18n.Tr("hpc.name");
    public static string Description => I18n.Tr("hpc.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsHPCRequest(MedicalGrantRequest request)
        => request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    // 减伤62.7%: 1/(1+a) = 0.373, a = 1/0.373 - 1 = 1.6808
    public static float WearableArmor = 1.6808f;
    public static float Weight = 2.1f;
    public static float WearableHitDurabilityLossMultiplier = 0.23f;
    public static float WearableIsolation = 0.04f;
    ...
    {
        if (!IsHPCRequest(request)) return;
        item.id = ItemKey;
        item.SetCondition(1f);
        var icon = TryLoadIcon();
        var sr = item.GetComponent<SpriteRenderer>();
        if (icon != null && sr != null) { sr.sprite = icon; Plugin.Log.LogInfo($"[HPC] Set sprite ({icon.texture.width}x{icon.texture.height})."); }
        else { Plugin.Log.LogWarning("[HPC] Icon load failed."); }
        ResizeColliderToSprite(item);
        Plugin.Log.LogInfo($"[HPC] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[HPC] Registered '{ItemKey}' as wearable armor.");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[HPC] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[HPC] CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}.");
    }
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "hpc.png");
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "hpc-icon";
            }
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[HPC] Icon: {ex.Message}"); }
        return _cachedIcon;
    }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "hpc-worn";
            }
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[HPC] Worn: {ex.Message}"); }
    ...
    public static class HPCHoverPatch
    {
        [HarmonyPostfix]
