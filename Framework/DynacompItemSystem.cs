...
/// <summary>
/// Spike Tactical Dynacomp 7.62x39 AK 膛口制退器【Dynacomp】。
/// 效果（由 SuppressorSystem.FireEffectsPatch 读取 GunAttachmentHolder 应用）：
/// - 后坐力 -8%（knockBack × 0.92）
/// - 瞄准速度 +0.05s（变慢，见 AimSystem.AttachmentAimTimeDelta）
/// 安装：AKM 专属枪口槽（与其他枪口装置互斥）。
/// </summary>
public static class DynacompItemSystem
{
    public const string ItemKey = "dynacomp";
    public const string BaseGameItemId = "bruisekit";

    public static string DisplayName => I18n.Tr("dynacomp.name");
    public static string Description => I18n.Tr("dynacomp.desc");

    public const float KnockBackMult = 0.92f;   // -8%
    ...
    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        ...
        Plugin.Log.LogInfo($"[Dynacomp] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[Dynacomp] Registered '{ItemKey}' as attachment (tag=attachment).");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[Dynacomp] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[Dynacomp] CUCoreLib: Icon={customInfo.Icon != null}.");
    }
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "guns", "akm", "dynacomp.png");
            if (File.Exists(iconPath))
            {
                ...
                if (ImageConversion.LoadImage(texture, bytes, false))
                {
                    ...
                    _cachedIcon.name = "dynacomp-icon";
                }
            }
            else Plugin.Log.LogWarning($"[Dynacomp] Icon not found: {iconPath}");
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[Dynacomp] Icon: {ex.Message}"); }
        return _cachedIcon;
    }
            ...
            if (File.Exists(iconPath))
            {
                ...
                if (ImageConversion.LoadImage(texture, bytes, false))
                {
                    ...
                    _cachedVisualIcon = Sprite.Create(texture,
                        new Rect(0, 0, texture.width, texture.height),
                        new Vector2(0.5f, 0.5f), 14f);
                    _cachedVisualIcon.name = "dynacomp-visual";
                }
            }
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[Dynacomp] Visual icon: {ex.Message}"); }
