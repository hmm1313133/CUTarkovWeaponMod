...
/// 半自动射击模式，12发弹匣供弹。
/// </summary>
public static class USPItemSystem
{
    public const string ItemKey = "usp";
    public const string BaseGameItemId = "pistol";

    public static string DisplayName => I18n.Tr("usp.name");
    public static string Description => I18n.Tr("usp.desc");
    ...
    public static bool IsUSPRequest(MedicalGrantRequest request)
        => request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        if (!IsUSPRequest(request)) return;
        ...
        if (gun != null)
        {
            ...
            var rackSound = TryLoadSound("usp_open", "usp");
            if (rackSound != null)
                gun.customRack = rackSound;
            var unrackSound = TryLoadSound("usp_close", "usp");
            if (unrackSound != null)
            ...
            Plugin.Log.LogInfo($"[USP] Configured GunScript: mag={MagCapacity}, dmg={AnimalDamage}, spread={VerticalSpread}, mode=SemiAuto");
        }
        ...
        Plugin.Log.LogInfo($"[USP] Configured spawned item '{ItemKey}' (condition={item.condition}).");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            if (Item.GlobalItems.TryGetValue(BaseGameItemId, out var source))
            {
                ...
                Plugin.Log.LogInfo($"[USP] Registered '{ItemKey}' (cloned from '{BaseGameItemId}').");
                return true;
            }

            Item.GlobalItems[ItemKey] = CreateFallbackItemInfo();
            Plugin.Log.LogInfo($"[USP] Registered '{ItemKey}' (fallback).");
            return true;
        }
        catch (Exception ex)
        {
            Plugin.Log.LogError($"[USP] Failed to register '{ItemKey}': {ex}");
            return false;
        }
    }
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "guns", "usp", "usp.png");
        ...
        {
            Plugin.Log.LogWarning($"[USP] Failed to load icon: {ex.Message}");
        }
    ...
    private static Sprite? TryLoadNoMagIcon()
    {
        ...
        {
            var assemblyDir = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) ?? Paths.PluginPath;
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "guns", "usp", "usp_magout.png");
        ...
        {
            Plugin.Log.LogWarning($"[USP] Failed to load no-mag icon: {ex.Message}");
        }
    ...
    private static AudioClip? TryLoadFireSound()
    {
        ...
        _cachedFireSound = TryLoadSound("usp_fire", "usp");
        if (_cachedFireSound != null)
            Plugin.Log.LogInfo("[USP] Loaded fire sound 'usp_fire.wav'");
    ...
    private static AudioClip? TryLoadSound(string fileName, string gunDir)
    {
        ...
        {
            Plugin.Log.LogWarning($"[USP] Failed to load sound '{fileName}': {ex.Message}");
        }
        return null;
    }
...
public sealed class USPItemMarker : MonoBehaviour
{
    public string displayName = USPItemSystem.DisplayName;
