...
/// <summary>
/// SilencerCo AC-858 ASR .338 LM 膛口制退器【AC-858】。
/// SilencerCo 研发的 .338 LM 膛口制退器。
///
/// 效果（由 SuppressorSystem.FireEffectsPatch 读取 GunAttachmentHolder 应用）：
/// - 后坐力 -23%（knockBack × 0.77）
/// - 开镜速度 +0.05s（AimSystem.AttachmentAimTimeDelta +0.05，减慢）
///
/// AXMC 专属枪口槽。
/// 交互：改枪面板（G 键）安装/卸下。
/// </summary>
public static class Ac858ItemSystem
{
    public const string ItemKey = "ac858";
    public const string BaseGameItemId = "bruisekit";

    public static string DisplayName => I18n.Tr("ac858.name");
    public static string Description => I18n.Tr("ac858.desc");

    public const float KnockBackMult = 0.77f;   // 后坐力 -23%
    public const float AimTimeDelta = 0.05f;    // 开镜速度 +0.05s（减慢）
    ...
    public static bool IsAc858Request(MedicalGrantRequest request)
        => request != null && request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        if (!IsAc858Request(request)) return;
        item.id = ItemKey;
        ...
        Plugin.Log.LogInfo($"[AC-858] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[AC-858] Registered '{ItemKey}' as attachment.");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[AC-858] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[AC-858] CUCoreLib: Icon={customInfo.Icon != null}.");
    }

    private static Sprite? TryLoadIcon()
    {
        if (_cachedIcon != null) return _cachedIcon;
        _cachedIcon = LoadSprite("ac858.png", "ac858-icon");
        return _cachedIcon;
    }
    ...
    private static Sprite? TryLoadVisualIcon()
    {
        ...
        _cachedVisualIcon = LoadSprite("ac858.png", "ac858-visual");
    ...
    private static Sprite? LoadSprite(string file, string name)
    {
        try
        {
            ...
            else Plugin.Log.LogWarning($"[AC-858] Icon not found: {iconPath}");
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[AC-858] Icon: {ex.Message}"); }
        return null;
    }
