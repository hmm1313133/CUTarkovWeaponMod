...
/// <summary>
/// Zenit RK-0 前握把【RK-0】。
/// 效果：后坐力 -1.7%（knockBack × 0.983）、开镜速度 -0.08 秒（AimSystem）。
/// 注意：RK-0 暂无专用贴图，临时复用 RK-2 同款 Zenit 垂直握把外观（rk0.png）。
/// </summary>
public static class Rk0ForegripItemSystem
{
    public const string ItemKey = "rk0";
    public const string BaseGameItemId = "bruisekit";

    public static string DisplayName => I18n.Tr("foregrip_rk0.name");
    public static string Description => I18n.Tr("foregrip_rk0.desc");

    public const float KnockBackMult = 0.983f; // 后坐力 -1.7%
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        var tex = TacticalLightHelper.LoadPointIconTexture("guns/common/rk0.png");
        if (tex != null)
        {
            _cachedIcon = Sprite.Create(tex,
                new Rect(0, 0, tex.width, tex.height),
                new Vector2(0.5f, 0.5f), 14f);
            _cachedIcon.name = "rk0-icon";
        }
        return _cachedIcon;
    }
