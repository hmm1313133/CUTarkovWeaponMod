...
/// <summary>
/// LBT-8005A Day Pack 背包（黑系复合迷彩）【Day Pack】
///
/// 简单可靠的黑色复合迷彩背包，占用 back 槽位，不提供防护。
...
/// </summary>
public static class DayPackItemSystem
{
    public const string ItemKey = "daypack";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "back";

    public static string DisplayName => I18n.Tr("daypack.name");
    public static string Description => I18n.Tr("daypack.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsDayPackRequest(MedicalGrantRequest request)
    ...
    {
        if (!IsDayPackRequest(request)) return;
        ...
        {
            sr.sprite = icon;
            Plugin.Log.LogInfo($"[DayPack] Set sprite to daypack icon ({icon.texture.width}x{icon.texture.height}).");
        }
        else
        {
            Plugin.Log.LogWarning($"[DayPack] Icon load failed - will keep base prefab sprite.");
        }

        ResizeColliderToSprite(item);

        Plugin.Log.LogInfo($"[DayPack] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[DayPack] Registered '{ItemKey}' as wearable backpack (no armor, decays over time, tearable).");
            return true;
        }
        catch (Exception ex)
        {
            Plugin.Log.LogError($"[DayPack] Failed to register '{ItemKey}': {ex}");
            return false;
        }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[DayPack] RegisterWithCUCoreLib: icon={icon != null}, wornIcon={wornIcon != null}");
        ...
        Plugin.Log.LogInfo($"[DayPack] Configured CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}, Container={customInfo.Container != null}.");
    }
    ...
    public static void TickDecay()
    {
        ...
        if (newCondition <= 0f)
        {
            Plugin.Log.LogInfo($"[DayPack] Backpack deteriorated to zero condition, destroying.");
            item.condition = 0f;
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "daypack.png");
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "daypack-icon";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[DayPack] Failed to load icon: {ex.Message}");
        }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "daypack-worn";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[DayPack] Failed to load worn icon: {ex.Message}");
        }
