...
/// <summary>
/// KGB MG-47 AK 手枪式握把（电镀红）【MG-47】。
///
/// 效果（由 SuppressorSystem.FireEffectsPatch 读取 GunAttachmentHolder 应用）：
/// - 后坐力 -2%（knockBack × 0.98）
///
/// 安装要求：
/// - 需要 Leatherman 工具钳（ToolSystem.AttachmentRequiresLeatherman）
/// - 占用后握把槽：与一体式枪托（CQR47）、RK-3 互斥，同一把枪只能装一个
/// - 无其他安装前提
///
/// 视觉：方案 A 纹理合成。位置与 RK-3 相同（覆盖原 AK 后握把区域）。
/// 交互：改枪面板（G 键）安装/卸下。
/// </summary>
public static class Mg47ItemSystem
{
    public const string ItemKey = "mg47";
    public const string BaseGameItemId = "bruisekit";

    public static string DisplayName => I18n.Tr("mg47.name");
    public static string Description => I18n.Tr("mg47.desc");
    ...
    public static bool IsMg47Request(MedicalGrantRequest request) =>
        request != null && request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        if (!IsMg47Request(request)) return;
        item.id = ItemKey;
        ...
        Plugin.Log.LogInfo($"[MG-47] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[MG-47] Registered '{ItemKey}' as attachment.");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[MG-47] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[MG-47] CUCoreLib: Icon={customInfo.Icon != null}.");
    }

    private static Sprite? TryLoadIcon()
    {
        if (_cachedIcon != null) return _cachedIcon;
        var tex = TacticalLightHelper.LoadPointIconTexture("guns/akm/mg47.png");
        if (tex != null)
        {
            _cachedIcon = Sprite.Create(tex,
                new Rect(0, 0, tex.width, tex.height),
                new Vector2(0.5f, 0.5f), 14f); // PPI 14 与枪械贴图一致
            _cachedIcon.name = "mg47-icon";
        }
        return _cachedIcon;
    }
