...
/// <summary>
/// SRVV 7.62x39 AK 膛口制退器【SRVV AKM】。
/// SRVV 为 7.62x39 口径 AK 及兼容武器系统制造的膛口制退器。
///
/// 效果（由 SuppressorSystem.FireEffectsPatch 读取 GunAttachmentHolder 应用）：
/// - 后坐力 -11%（knockBack × 0.89）
/// - 瞄准速度 +0.12s（AimSystem.AttachmentAimTimeDelta +0.12，减慢）
///
/// AKM 专属枪口槽；SKS 可通过 WT0032-1 转接器安装。
/// 交互：改枪面板（G 键）安装/卸下。
/// </summary>
public static class SrvvAkmItemSystem
{
    public const string ItemKey = "srvvakm";
    public const string BaseGameItemId = "bruisekit";

    public static string DisplayName => I18n.Tr("srvvakm.name");
    public static string Description => I18n.Tr("srvvakm.desc");

    public const float KnockBackMult = 0.89f;   // 后坐力 -11%
    ...
    public static bool IsSrvvAkmRequest(MedicalGrantRequest request)
        => request != null && request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        if (!IsSrvvAkmRequest(request)) return;
        item.id = ItemKey;
        ...
        Plugin.Log.LogInfo($"[SRVV AKM] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[SRVV AKM] Registered '{ItemKey}' as attachment.");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[SRVV AKM] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[SRVV AKM] CUCoreLib: Icon={customInfo.Icon != null}.");
    }

    private static Sprite? TryLoadIcon()
    {
        if (_cachedIcon != null) return _cachedIcon;
        _cachedIcon = LoadSprite("srvvakm.png", "srvvakm-icon");
        return _cachedIcon;
    }
    ...
    private static Sprite? TryLoadVisualIcon()
    {
        ...
        _cachedVisualIcon = LoadSprite("srvvakm.png", "srvvakm-visual");
    ...
    private static Sprite? LoadSprite(string file, string name)
    {
        try
        {
            ...
            else Plugin.Log.LogWarning($"[SRVV AKM] Icon not found: {iconPath}");
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[SRVV AKM] Icon: {ex.Message}"); }
        return null;
    }
