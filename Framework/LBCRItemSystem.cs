...
/// <summary>
/// LBT-1961A 承重胸挂（MAS 灰色）
///
/// 战术承重系统，占用 bandolier 槽位，不提供防护。
/// 穿戴时持续消耗耐久，约7.5小时后损坏（destroyAtZeroCondition = true）。
/// 通过 CUCoreLib CustomItemInfo 注册，设置 wearable + Container + WornSprite。
/// </summary>
public static class LBCRItemSystem
{
    public const string ItemKey = "lbcr";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "bandolier";

    public static string DisplayName => I18n.Tr("lbcr.name");
    public static string Description => I18n.Tr("lbcr.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsLBCRRequest(MedicalGrantRequest request)
        => request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    // === 数值 ===
    public static float Weight = 1.6f;                    // 重量 1.6u
    public static float WearableIsolation = 0.02f;        // 保温值
    public static int Value = 36;                          // 价值
    ...
    {
        if (!IsLBCRRequest(request)) return;
        ...
        {
            sr.sprite = icon;
            Plugin.Log.LogInfo($"[LBCR] Set sprite to lbcr icon ({icon.texture.width}x{icon.texture.height}).");
        }
        else
        {
            Plugin.Log.LogWarning($"[LBCR] Icon load failed - will keep base prefab sprite.");
        }

        ResizeColliderToSprite(item);

        Plugin.Log.LogInfo($"[LBCR] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        {
            Plugin.Log.LogError($"[LBCR] Failed to register '{ItemKey}': {ex}");
            return false;
        }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[LBCR] RegisterWithCUCoreLib: icon={icon != null}, wornIcon={wornIcon != null}");
        ...
        Plugin.Log.LogInfo($"[LBCR] Configured CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}, Container={customInfo.Container != null}.");
    }
    ...
    public static void TickDecay()
    {
        ...
        if (newCondition <= 0f)
        {
            Plugin.Log.LogInfo($"[LBCR] Chest rig deteriorated to zero condition, destroying.");
            item.condition = 0f;
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "LBCR.png");
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "lbcr-icon";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[LBCR] Failed to load icon: {ex.Message}");
        }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "lbcr-worn";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[LBCR] Failed to load worn icon: {ex.Message}");
        }
