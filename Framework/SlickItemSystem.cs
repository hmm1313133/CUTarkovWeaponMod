...
public static class SlickItemSystem
{
    public const string ItemKey = "slick";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "outertorso";

    public static string DisplayName => I18n.Tr("slick.name");
    public static string Description => I18n.Tr("slick.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsSlickRequest(MedicalGrantRequest request)
    ...
    {
        if (!IsSlickRequest(request)) return;
        item.id = ItemKey;
        item.SetCondition(1f);
        var icon = TryLoadIcon();
        var sr = item.GetComponent<SpriteRenderer>();
        if (icon != null && sr != null) { sr.sprite = icon; Plugin.Log.LogInfo($"[Slick] Set sprite ({icon.texture.width}x{icon.texture.height})."); }
        else { Plugin.Log.LogWarning("[Slick] Icon load failed."); }
        ResizeColliderToSprite(item);
        Plugin.Log.LogInfo($"[Slick] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[Slick] Registered '{ItemKey}' as wearable armor.");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[Slick] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[Slick] CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}.");
    }
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "slick.png");
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "slick-icon";
            }
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[Slick] Icon: {ex.Message}"); }
        return _cachedIcon;
    }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "slick-worn";
            }
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[Slick] Worn: {ex.Message}"); }
