...
/// <summary>
/// CGS Hekate DT .338 LM 消音器【Hekate DT .338】。
/// Hekate DT .338 LM 消音器能够有效降低大口径步枪开火时的声音与火光，减少射击特征，由 CGS 集团设计制造。
///
/// 效果（由 SuppressorSystem.FireEffectsPatch 读取 GunAttachmentHolder 应用）：
/// - 后坐力 -5%（knockBack × 0.95）
/// - 听力损伤 -65%（loudness × 0.35）
/// - 开镜速度 +0.35s（AimSystem.AttachmentAimTimeDelta +0.35，减慢）
/// - 取消枪口火光
///
/// AXMC 专属枪口槽。
/// 交互：改枪面板（G 键）安装/卸下。
/// </summary>
public static class HekateDt338ItemSystem
{
    public const string ItemKey = "hekate_dt338";
    public const string BaseGameItemId = "bruisekit";

    public static string DisplayName => I18n.Tr("hekate_dt338.name");
    public static string Description => I18n.Tr("hekate_dt338.desc");

    public const float KnockBackMult = 0.95f;   // 后坐力 -5%
    public const float LoudnessMult = 0.35f;    // 听力损伤 -65%
    public const float AimTimeDelta = 0.35f;    // 开镜速度 +0.35s（减慢）
    ...
    public static bool IsHekateDt338Request(MedicalGrantRequest request)
        => request != null && request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        if (!IsHekateDt338Request(request)) return;
        item.id = ItemKey;
        ...
        Plugin.Log.LogInfo($"[Hekate DT] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[Hekate DT] Registered '{ItemKey}' as attachment.");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[Hekate DT] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[Hekate DT] CUCoreLib: Icon={customInfo.Icon != null}.");
    }

    private static Sprite? TryLoadIcon()
    {
        if (_cachedIcon != null) return _cachedIcon;
        _cachedIcon = LoadSprite("hekate dt.338.png", "hekate-dt338-icon");
        return _cachedIcon;
    }
    ...
    private static Sprite? TryLoadVisualIcon()
    {
        ...
        _cachedVisualIcon = LoadSprite("hekate dt.338.png", "hekate-dt338-visual");
    ...
    private static Sprite? LoadSprite(string file, string name)
    {
        try
        {
            ...
            else Plugin.Log.LogWarning($"[Hekate DT] Icon not found: {iconPath}");
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[Hekate DT] Icon: {ex.Message}"); }
        return null;
    }
