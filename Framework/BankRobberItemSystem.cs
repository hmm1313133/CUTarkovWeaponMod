...
/// <summary>
/// Spiritus Systems Bank Robber 胸挂（高原复合迷彩）
///
/// 轻便战术胸挂，占用 bandolier 槽位，不提供防护。
/// 穿戴时持续消耗耐久，约6小时后损坏（destroyAtZeroCondition = true）。
/// 通过 CUCoreLib CustomItemInfo 注册，设置 wearable + Container + WornSprite。
/// </summary>
public static class BankRobberItemSystem
{
    public const string ItemKey = "bankrobber";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "bandolier";

    public static string DisplayName => I18n.Tr("bankrobber.name");
    public static string Description => I18n.Tr("bankrobber.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsBankRobberRequest(MedicalGrantRequest request)
        => request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    // === 数值 ===
    public static float Weight = 0.6f;                    // 重量 0.6u
    public static float WearableIsolation = 0.02f;        // 保温值
    public static int Value = 24;                          // 价值
    public static int RecognitionMin = 3;                  // 识别所需智力
    public static float ContainerCapacity = 2.5f;          // 容器容量 2.5u
    public static float ContainerMaxWeightPerItem = 1f;    // 单物品最大重量 1u
    public static float ContainerEncumbranceReduction = 0.54f; // 重量减免 46%
    ...
    {
        if (!IsBankRobberRequest(request)) return;
        ...
        {
            sr.sprite = icon;
            Plugin.Log.LogInfo($"[BankRobber] Set sprite to bankrobber icon ({icon.texture.width}x{icon.texture.height}).");
        }
        else
        {
            Plugin.Log.LogWarning($"[BankRobber] Icon load failed - will keep base prefab sprite.");
        }

        ResizeColliderToSprite(item);

        Plugin.Log.LogInfo($"[BankRobber] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[BankRobber] Registered '{ItemKey}' as wearable bandolier (no armor, decays over time).");
            return true;
        }
        catch (Exception ex)
        {
            Plugin.Log.LogError($"[BankRobber] Failed to register '{ItemKey}': {ex}");
            return false;
        }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[BankRobber] RegisterWithCUCoreLib: icon={icon != null}, wornIcon={wornIcon != null}");
        ...
        Plugin.Log.LogInfo($"[BankRobber] Configured CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}, Container={customInfo.Container != null}.");
    }
    ...
    public static void TickDecay()
    {
        ...
        if (newCondition <= 0f)
        {
            Plugin.Log.LogInfo($"[BankRobber] Chest rig deteriorated to zero condition, destroying.");
            item.condition = 0f;
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "bankrobber.png");
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "bankrobber-icon";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[BankRobber] Failed to load icon: {ex.Message}");
        }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "bankrobber-worn";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[BankRobber] Failed to load worn icon: {ex.Message}");
        }
    ...
    public static class BankRobberHoverPatch
    {
        [HarmonyPostfix]
