...
public static class ExfilItemSystem
{
    public const string ItemKey = "exfil";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "hat";

    public static string DisplayName => I18n.Tr("exfil.name");
    public static string Description => I18n.Tr("exfil.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsExfilRequest(MedicalGrantRequest request)
    ...
    public static int Value = 46;
    public static int RecognitionMin = 7;
    public static int WearableVisualOffset = 8;

    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        if (!IsExfilRequest(request)) return;
        item.id = ItemKey;
        item.SetCondition(1f);
        var icon = TryLoadIcon();
        var sr = item.GetComponent<SpriteRenderer>();
        if (icon != null && sr != null) { sr.sprite = icon; Plugin.Log.LogInfo($"[EXFIL] Set sprite ({icon.texture.width}x{icon.texture.height})."); }
        else { Plugin.Log.LogWarning("[EXFIL] Icon load failed."); }
        ResizeColliderToSprite(item);
        Plugin.Log.LogInfo($"[EXFIL] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        try
        {
            ...
            Plugin.Log.LogInfo($"[EXFIL] Registered '{ItemKey}' as wearable helmet.");
            return true;
        }
        catch (Exception ex) { Plugin.Log.LogError($"[EXFIL] Failed: {ex}"); return false; }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[EXFIL] CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}.");
    }
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "exfil.png");
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "exfil-icon";
            }
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[EXFIL] Icon: {ex.Message}"); }
        return _cachedIcon;
    }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "exfil-worn";
            }
        }
        catch (Exception ex) { Plugin.Log.LogWarning($"[EXFIL] Worn: {ex.Message}"); }
    ...
    public static class ExfilHoverPatch
    {
        [HarmonyPostfix]
