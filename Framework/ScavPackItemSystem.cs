...
/// Scav背包【Scavpack】
///
/// 山寨了某款 3 日突击背包的便宜货，占用 back 槽位，不提供防护。
/// 穿戴时持续消耗耐久，约5小时后损坏（destroyAtZeroCondition = true）。
/// 拥有5点可撕裂属性（wearableHitDurabilityLossMultiplier = 5），受击时快速损耗。
/// 通过 CUCoreLib CustomItemInfo 注册，设置 wearable + Container + WornSprite。
/// 合成配方与原版 bigpack 相同（2绳+1木板+5帆布+3线+切割工具，INT=13）。
/// </summary>
public static class ScavPackItemSystem
{
    public const string ItemKey = "scavpack";
    public const string BaseGameItemId = "bruisekit";
    public const string WearSlotId = "back";

    public static string DisplayName => I18n.Tr("scavpack.name");
    public static string Description => I18n.Tr("scavpack.desc");

    private static Sprite? _cachedIcon;
    private static Sprite? _cachedWornIcon;

    public static bool IsScavPackRequest(MedicalGrantRequest request)
        => request.ItemKey.Equals(ItemKey, StringComparison.OrdinalIgnoreCase);

    // === 数值 ===
    public static float Weight = 0.9f;                        // 重量 0.9u
    public static float WearableIsolation = 0.02f;            // 保温值
    public static int Value = 25;                              // 价值
    ...
    // === 时间衰减 ===
    // 约5小时损坏：5 * 3600 = 18000 秒，每秒衰减 1/18000
    public static float DecayRatePerSecond = 1.0f / 18000.0f;

    public static void ConfigureSpawnedItem(Item item, MedicalGrantRequest request)
    {
        if (!IsScavPackRequest(request)) return;
        ...
        {
            sr.sprite = icon;
            Plugin.Log.LogInfo($"[ScavPack] Set sprite to scavpack icon ({icon.texture.width}x{icon.texture.height}).");
        }
        else
        {
            Plugin.Log.LogWarning($"[ScavPack] Icon load failed - will keep base prefab sprite.");
        }

        ResizeColliderToSprite(item);

        Plugin.Log.LogInfo($"[ScavPack] Configured spawned item '{ItemKey}'.");
    }
    ...
    public static bool EnsureRegisteredInItemTable()
    {
        ...
        {
            Plugin.Log.LogError($"[ScavPack] Failed to register '{ItemKey}': {ex}");
            return false;
        }
    }
    ...
    public static void RegisterWithCUCoreLib(CustomItemInfo customInfo)
    {
        ...
        Plugin.Log.LogInfo($"[ScavPack] RegisterWithCUCoreLib: icon={icon != null}, wornIcon={wornIcon != null}");
        ...
        Plugin.Log.LogInfo($"[ScavPack] Configured CUCoreLib: WornSprite={customInfo.WornSprite != null}, Icon={customInfo.Icon != null}, Container={customInfo.Container != null}.");
    }
    ...
    public static void TickDecay()
    {
        ...
        if (newCondition <= 0f)
        {
            Plugin.Log.LogInfo($"[ScavPack] Backpack deteriorated to zero condition, destroying.");
            item.condition = 0f;
    ...
    private static Sprite? TryLoadIcon()
    {
        ...
        try
        {
            ...
            var iconPath = Path.Combine(assemblyDir, "Framework", "Assets", "equipment", "scavpack.png");
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedIcon.name = "scavpack-icon";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[ScavPack] Failed to load icon: {ex.Message}");
        }
    ...
    private static Sprite? TryLoadWornIcon()
    {
        ...
        try
        {
            ...
            if (File.Exists(iconPath))
            {
                ...
                _cachedWornIcon.name = "scavpack-worn";
            }
        }
        catch (Exception ex)
        {
            Plugin.Log.LogWarning($"[ScavPack] Failed to load worn icon: {ex.Message}");
        }
    ...
    public static class ScavPackHoverPatch
    {
        [HarmonyPostfix]
