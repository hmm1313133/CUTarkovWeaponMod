...
- **配件系统全面扩展**
  - Leatherman 多功能工具钳（`leatherman`）：重量 0.5u，价值 20，可合成
  - Magpul MOE AKM 护木（`moeakm`）：后坐力 -1%、每发耐久损耗 -3%，需工具钳，是握把/战术灯/激光安装前提
  - TDI AKM-L 护木（`akml`）：瞄准速度 -0.35s、每发耐久损耗 -3%，需工具钳
  - Spike Tactical Dynacomp（`dynacomp`）：后坐力 -8%、瞄准速度 +0.05s
  - Zenit DTK-1（`dtk1`）：后坐力 -12%、瞄准速度 +0.1s、无枪口火光
  - AKM/M4/SKS/Glock/P90/UMP 多系列护木、枪口、瞄具、战术设备、握把、枪托、基座、套筒等配件与局部叠加视觉
- **配件工具钳与前提机制**
  - `ToolSystem` 提供工具钳检测、配件依赖前置管理
  - 改枪面板无工具/缺前提时按钮禁用并显示提示
  - 拖拽安装同样校验工具钳
- **配件视觉局部叠加**
  - 透明背景配件局部贴图，任意数量配件可同时显示
- **武器配件地堡**
  - Weapon Area：35x12 实验室地堡，刷卡进入，室内小/大武器箱，大箱 30% 掉枪、掉 2~4 配件
  - Blue Area：医疗地堡，SMU06 医疗包、食物箱、自动医疗系统
  - Red Area：高级装备地堡，大型武器箱、装备物资箱、子弹箱
  - 双层防弹加强钢墙体，防爆/防切割补丁，第五地层不刷新
- **独立 `vanillablock` 控制台命令**
  - `list / on / off / [物品] true|false`
  - 支持 Tab 自动补全与单物品覆盖
- **多人同步重做**
  - 配件安装/卸下双向同步，客户端与主机互相可见
  - 自定义弹匣 `currentMagId` 同步，主机装弹匣后客户端不再显示默认贴图/空弹匣
  - 战术手电/激光开关与档位同步，他人可看到灯光/激光
  - 主机每 3 秒广播全量枪械状态，重连/后加入玩家可恢复改装、弹匣、电量
  - 存档扩展：原版/QoL 存档保存配件、弹匣、弹药数、电量、无电池标记
- **瞄准速度面板精确显示**
  - 改枪面板/悬停显示瞄准时间精确到 0.01s

### 变更

- **瞄准速度平衡调整**
  - AKM 30 发弹匣瞄准 +0.2s（变慢）
  - X-47 50 发弹鼓 +0.5s（变慢）
  - TBL +0.1s、Klesch-2U +0.25s、Baldr Pro +0.18s、LAS/TAC 2 +0.15s
  - Razor HD +0.5s、PM II +0.7s（由加快改为变慢）
- **弹匣价值差异化**
  - 按容量计价，不再统一为 2
- **配件视觉**
  - 改为局部叠加方案，多配件可同时显示
- **数值与文案修正**
  - 枪械重量、伤害、噪音等按 2.0 当前版本校准
  - AKM 战术灯贴图不再误套手枪偏移
  - UMP45 文案修正为“可装 UMP OEM 消音器”
...
- 多人：
  - 客户端卸配件不再掉到主机脚边
  - 普通配件卸下后不再变成 1% 耐久
  - 主机装弹匣后客户端显示正确弹匣贴图/弹匣检查
  - 他人手上的激光不再反向
  - 重连/后加入玩家枪械不再丢失配件
- 配件视觉、弹匣贴图、战术灯位置
- 存档恢复后控制器与枪械视觉重新挂载
- 合成、世界生成、语言切换、背包/护甲/头盔耐久等多项问题
